...
четыре обязательных прохода и подтверждён ожидаемый shader contract. Первый
кадр полной цепочки всегда остаётся Vanilla. Addon возвращается к Vanilla при:

- неподдерживаемой версии DOF-шейдеров;
...
one frame and the expected shader contract is confirmed. The first complete
chain frame is always Vanilla. The addon returns to Vanilla on:

- an unsupported DOF shader revision;
